...
// @version      0.3.1
// @description  Intercept _blank links, Cmd/Ctrl/middle clicks and window.open on multica.ai, then open them as native new tabs via the app's window tabbing.
// @author       sanyi
// @match        http://*/*
// @match        https://*/*
...
(function () {
  ...
  function shouldInterceptAnchor(event, anchor) {
    ...
    if (!isMulticaUrl(anchor.href)) return false;
  ...
  // Only reroute multica.ai calls that ask for a new browsing context with a
  // usable URL (window.open(url, "_blank") and window.open(url) with no
  // target). Everything else — other domains, no URL, named targets — keeps
  // the native behavior untouched.
  window.open = function (url, target, features) {
    if (url === undefined || url === null || String(url) === "") {
      return nativeOpen(url, target, features);
    }
    if (!isMulticaUrl(url)) return nativeOpen(url, target, features);
    const wantsNewContext =
      target === undefined || target === null || String(target).trim() === "" ||
      String(target).trim().toLowerCase() === "_blank";
    if (!wantsNewContext) return nativeOpen(url, target, features);
    return nativeOpen(String(url), "_blank", "noopener");
  };

  // Hold Alt while clicking a link to bypass the interception.
  // --- Original Arc-style iframe preview panel (v0.2.2), disabled in favor of
  // --- native window tabs. Restore by uncommenting and removing the block above.
